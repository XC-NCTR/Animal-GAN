...
parser.add_argument("--Weight_dim", type=int, default=1, help="dimension of terminal body weight")
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            *block(opt.Z_dim + opt.Stru_dim + opt.Time_dim + opt.Dose_dim + opt.Weight_dim, 4096, normalize=False),
            *block(4096, 2048),
            *block(2048, 1024),
            *block(1024, 512),
            nn.Linear(512, opt.Measurement_dim),
            nn.Tanh()
        )

    def forward(self, noise, Stru, Time, Dose, Weight):
        # Concatenate conditions and noise to produce input
        gen_input = torch.cat([noise, Stru, Time, Dose, Weight], -1)
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            nn.Linear(opt.Stru_dim + opt.Time_dim + opt.Dose_dim + opt.Weight_dim + opt.Measurement_dim, 2048),
    ...
    def forward(self, Measurement, Stru, Time, Dose, Weight):
        # Concatenate conditions and real_Measurement to produce input
        d_in = torch.cat((Measurement, Stru, Time, Dose, Weight), -1)
...
Data = Data.iloc[:, 0:4].join(pd.DataFrame(MinMaxScaler().fit_transform(Data.iloc[:, 4:]), columns=Data.columns[4:]))

# Measurements = Data[Data.columns[5:]]
S = pd.DataFrame(columns=Stru.columns)
M = pd.DataFrame(columns=Data.columns[5:])
...
def compute_gradient_penalty(Dis, real_samples, fake_samples, Stru, Time, Dose, Weight):
    """Calculates the gradient penalty loss for WGAN GP"""
    # Random weight term for interpolation between real and fake samples
    alpha = torch.rand((real_samples.size(0), 1)).to(device)
    # Get random interpolation between real and fake samples
    interpolates = (alpha * real_samples + ((1 - alpha) * fake_samples)).requires_grad_(True)
    d_interpolates = Dis(interpolates, Stru, Time, Dose, Weight)
...
for epoch in range(opt.n_epochs):
    # start = time.time()
    for i, (Stru, Measurement, Time, Dose, Weight) in enumerate(dataloader):
        ...
        gen_Measurement = generator(z, Stru, Time, Dose, Weight)

        validity_real = discriminator(Measurement, Stru, Time, Dose, Weight)
        # # Loss for real Exp
        # d_real_loss = adversarial_loss(validity_real, valid)

        validity_fake = discriminator(gen_Measurement.detach(), Stru, Time, Dose, Weight)
        # # Loss for fake Exp
        ...
        if (epoch * len(dataloader) + i) % opt.n_critic == 0:
            ...
            validity = discriminator(gen_Measurement, Stru, Time, Dose, Weight)
