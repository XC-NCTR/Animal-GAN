...
model_path = r'../model_2'  # please change it to the folder where your well-trained model save
weights = torch.load(os.path.join(model_path, 'generator'))
generator = Generator()
if cuda:
    generator.cuda()
...
genProfile = torch.tensor(()).to(device)
with torch.no_grad():
    for i in range(100):
        z = torch.randn(S.shape[0], opt.Z_dim).to(device)
        genProfile = torch.cat((genProfile, generator(z, S, T, D)))

Data = pd.read_csv("../Data/Data.tsv", sep="\t", index_col=0).T
scaler = MinMaxScaler()
scaler.fit_transform(Data)
df = pd.DataFrame(scaler.inverse_transform(genProfile))
np.save(os.path.join(path, 'Results', 'gen_model_2_100.npy'), scaler.inverse_transform(genProfile))
